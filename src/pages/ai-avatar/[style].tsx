...
interface AIGeneratorStylePageProps {
  locale?: string;
  style: AvatarStyle;
}

export default function AIGeneratorStylePage({
  locale: propLocale,
  style,
}: AIGeneratorStylePageProps) {
  const { t } = useTranslation('common');
  const router = useRouter();
  const pagePath = `/ai-avatar/${style}`;
  ...
  const pageTitle = `${t(`ai.styleTitles.${style}`)} | ${t('siteTitle')}`;
  const pageDescription = t('ai.description'); // Could be customized per style if needed
  ...
  return (
    <>
      ...
      <div className="min-h-screen flex flex-col bg-[#fffefc]">
        <Header />
        <AIGeneratorContent avatarStyle={style} />
        <Footer />
      </div>
    </>
  );
...
export const getStaticProps = async ({
  ...
  const style = styleParam?.toLowerCase();
  const validStyles = ['notion', 'ghibli', 'oil_painting'];
  ...
  return {
    props: {
      ...
      locale: locale ?? 'en',
      style, // Pass the normalized lowercase style
    },
  };
};
